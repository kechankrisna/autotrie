...
class _TrieSearchTree {
  ...
  bool _delete(TrieNode? cur, String word, int index) {
    ...
    var next = cur!.children![
        cur.children!.indexOf(TrieNode(ch, index == word.length - 1))];
    if (next == null) {
      return false;
    }
    var shouldDeleteNode = _delete(next, word, index + 1) &&
        (!(next.hits > 0) || index == word.length - 1);
...
class TrieNode {
  ...
  bool operator ==(Object other) {
    ...
    return other is TrieNode && value == other.value;
  }
}
